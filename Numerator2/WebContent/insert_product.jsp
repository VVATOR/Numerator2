	...
	if(!NEWOBOZ.equals(""))
		if(!NAIM.equals(""))
			if(!NEWFULLNAME.equals("")){
				sql.InsertStatistic("1",FULLNAME,NAIM,NEWOBOZ);
				sql.InsertIzdelie(NEWOBOZ,NAIM,NEWFULLNAME);
			}
...
<body id="dt_example"  background="images/background.png">
	<div class="full_width big" align=center>Создание нового изделия</div>
	...
	<br>
	<br>
	<u><font color="#FF0000">Тут вводятся только изделия.</font></u><i><font color="#336699"> Для ввода узла обратитесь к ведущему конструктору (или его доверенным пользователям), чтобы он создал соответствующее изделие и крупные узлы.</font></i><font color="#336699">
	</font>	
	<h1>Обазательные параметры</h1>
	<form action="insert_product.jsp" method="GET" name=form1>
	  <input type="text" value="1" name="Action" style="display: none;" />  
	  <table border=0>
			<tr>
				<!-- ОБОЗНАЧЕНИЕ -->
				<td><font color="#6495ED">Обозначение</font></td>
				<td><input type="text" size="20" name="NEWOBOZ" id="NEWOBOZ"/></td>
				<!-- НАИМЕНОВАНИЕ -->
				<td><font color="#6495ED">Наименование</font></td>
				<td><input type="text" name=NAIM id=NAIM size="60">
				<%
	...
	<table border=0 width=70% align=center>
	 	<tr>
	 		<td width=100><img name=pict2 border=0 onClick="AddExecute()" src="images/BUTTON_ADD.PNG" onMouseOver="changeImg2('images/BUTTON_ADD_SELECTED')" onMouseOut="changeImg2('images/BUTTON_ADD')"></td>
	 		<td width=100% align=right><img name=pict3 border=0 onClick="AddCancel()" src="images/BUTTON_CANCEL.PNG" onMouseOver="changeImg3('images/BUTTON_CANCEL_SELECTED')" onMouseOut="changeImg3('images/BUTTON_CANCEL')"></td>
	 	</tr>
 	</table>
